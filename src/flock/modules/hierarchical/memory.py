...
from flock.core import FlockModuleConfig
...
class HierarchicalMemoryModuleConfig(FlockModuleConfig):
...
class HierarchicalMemoryGraph(BaseModel):
    ...
    def __init__(self, **data):
        ...
        try:
            ...
            self._graph = json_graph.node_link_graph(
                data_graph, edges="links", multigraph=True
            )
            logger.debug(
            ...
            logger.error(
                f"Failed to load HierarchicalMemoryGraph from JSON: {e}"
            )
            ...
            for c2 in concepts:
                if c1 != c2:
                    ...
                    if self._graph.has_edge(
                        c1, c2, key=ConceptRelationType.ASSOCIATION
                    ):
                        # Increase weight of existing association
                        self._graph[c1][c2][ConceptRelationType.ASSOCIATION][
                            "weight"
                        ] += 1
                    else:
    ...
    def add_hierarchical_relation(
        ...
        if not self._graph.has_edge(
            child_concept, parent_concept, key=relation_type
        ):
            self._graph.add_edge(
                child_concept,
                parent_concept,
            ...
            logger.debug(
                f"Added {relation_type} relationship: {child_concept} -> {parent_concept}"
            )
        else:
            # Update weight if relationship already exists
            curr_weight = self._graph[child_concept][parent_concept][
                relation_type
            ]["weight"]
            self._graph[child_concept][parent_concept][relation_type][
                "weight"
            ] = curr_weight + weight
            logger.debug(
                f"Updated {relation_type} relationship: {child_concept} -> {parent_concept}"
            )
    ...
    def get_parents(
        ...
        parents = []
        for _, parent, key, data in self._graph.out_edges(
            concept, keys=True, data=True
        ):
    ...
    def get_children(
        ...
        children = []
        for child, _, key, data in self._graph.in_edges(
            concept, keys=True, data=True
        ):
            if key == relation_type:
    ...
    def hierarchical_spread_activation(
        ...
        """
        logger.debug(
            f"Hierarchical spreading activation from concepts: {initial_concepts}"
        )
        activated = {concept: 1.0 for concept in initial_concepts}
        frontier = [
            (concept, 0) for concept in initial_concepts
        ]  # (concept, depth)
        ...
        while frontier:
            ...
            for _, neighbor, key, edge_data in self._graph.out_edges(
                ...
                if relation_type == ConceptRelationType.ASSOCIATION:
                    ...
                    ConceptRelationType.INSTANCE_OF,
                ):
            ...
            for source, _, key, edge_data in self._graph.in_edges(
                current, keys=True, data=True
            ):
                relation_type = edge_data.get(
                    "relation_type", ConceptRelationType.ASSOCIATION
                ...
                if relation_type in (
                    ...
                    if (
                        source not in activated
                        or activated[source] < new_activation
                    ):
    ...
    def spread_activation(
        ...
        return self.hierarchical_spread_activation(
            initial_concepts, decay_factor=decay_factor
        )
    ...
    def save_as_image(self, filename: str = "memory_graph.png") -> None:
        ...
        logger.info(
            f"Saving HierarchicalMemoryGraph visualization to '{filename}'"
        )

        if self._graph.number_of_nodes() == 0:
            logger.warning(
                "HierarchicalMemoryGraph is empty; skipping image creation."
            )
            return
        ...
        try:
            ...
            logger.info(
                f"HierarchicalMemoryGraph image saved successfully to '{filename}'"
            )
...
class HierarchicalFlockMemoryStore(BaseModel):
    ...
    concept_graph: HierarchicalMemoryGraph = Field(
        default_factory=HierarchicalMemoryGraph
    )
    ...
    def exact_match(self, inputs: dict[str, Any]) -> list[MemoryEntry]:
        ...
        for entry in self.short_term:
            ...
            try:
                ...
                if isinstance(entry_data, dict) and all(
                    k in entry_data and entry_data[k] == v
                ...
                if all(
                    f"{k}.*?{v}" in entry.content for k, v in inputs.items()
                ):
                    matches.append(entry)
    ...
    def retrieve(
        ...
        with tracer.start_as_current_span("memory.retrieve") as span:
            logger.debug(
                "Retrieving memory entries with hierarchical support..."
            )
            results = []
            ...
            if use_hierarchical and query_concepts:
                if len(query_concepts) > 0:
                    concept_activations = (
                        self.concept_graph.hierarchical_spread_activation(
                            query_concepts,
                            decay_factor=0.5,
                ...
                concept_activations = (
                    self.concept_graph.spread_activation(
            ...
            logger.debug(
                f"Retrieved {len(results)} memory entries with hierarchical support."
            )
