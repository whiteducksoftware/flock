...
class Flock(BaseModel, Serializable):
    ...
    def run(
        ...
        if asyncio.get_event_loop() is loop and not loop.is_running():
            ...
            # Only close loop if we created it and it's not running anymore
            # loop.close() # Avoid closing if it might be used elsewhere
            return result
        else:
            # If using an existing loop, just run the coroutine
            # This assumes the existing loop is managed elsewhere
            ...
            # This might need adjustment depending on how the outer loop is managed
            # If run from another async context, just `await self.run_async(...)` is better
