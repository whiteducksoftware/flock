...
from flock.core.output_themes import OutputTheme

# Import the new hierarchical memory components
from hierarchical_memory_implementation import ConceptRelationType, MemoryModuleConfig
from hierarchical_memory_module import HierarchicalMemoryModule
...
async def main():
    ...
    memory_module = HierarchicalMemoryModule(name="hierarchical_memory", config=memory_config)
    ...
    agent.evaluator = memory_module
    
    # Initialize the agent
    await memory_module.initialize(agent, {})
    
    # Add some initial data with implicit hierarchies
    print("Adding initial information...")
    await agent.run(inputs={"data": "Andre is 38 years old and author of the agent framework 'flock'"})
    await agent.run(inputs={"data": "Andre works for White Duck"})
    await agent.run(inputs={"data": "Andre has two cats"})
    await agent.run(inputs={"data": "White Duck is a cloud consulting company"})
    await agent.run(inputs={"data": "Flock is an agent framework designed for scalable multi-agent systems"})
    
    # Add data about Andre's cats with names
    await agent.run(inputs={"data": "One of Andre's cats is named Luna"})
    await agent.run(inputs={"data": "The other cat is named Lucy"})
    
    # Add location information
    await agent.run(inputs={"data": "Andre lives in Germany"})
    await agent.run(inputs={"data": "Germany is in Europe"})
