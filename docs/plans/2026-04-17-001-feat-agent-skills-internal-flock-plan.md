...
- **Error hierarchy roots at `FlockError`** (verify import path at Unit 1 start; codebase grep showed no `class FlockError` — likely needs to be introduced under `src/flock/core/errors.py` or `SkillError(Exception)` directly). `SkillError(FlockError)` plus 9 subclasses: 7 from origin §9 minus `SkillTokenBudgetError` (zero raise sites — round-1 P2 #16) plus `SkillEngineModeError` (round-2 P1 #6 — raised inside `DSPyEngine.evaluate` when `_choose_program` silently degrades a tool-mode skill agent to Predict) plus `SkillTrainsetTooThinError` (round-2 P1 #5 — raised by `optimize.trainset.build_from_changelog` when no-upstream drop ratio exceeds threshold) plus `SkillDemoShapeError` (round-4 P1 #3 — raised by `load_demos_for_skill` when a demo line violates the portable `{input, output}` shape or cannot be remapped to the target agent's signature). Net: 9 subclasses with concrete raise sites each.
...
├── errors.py                    # SkillError hierarchy (9 exception classes — drop SkillTokenBudgetError per round-1 P2 #16, add SkillEngineModeError + SkillTrainsetTooThinError per round-2 P1, add SkillDemoShapeError per round-4 P1)
...
- Error hierarchy: `SkillError(FlockError)` root + 9 subclasses per origin §9 (drop `SkillTokenBudgetError` per round-1 P2 #16 — zero raise sites; add `SkillEngineModeError` per round-2 P1 #6 — raised when a tool-mode skill agent is silently degraded to Predict by `_choose_program` exception fallback; add `SkillDemoShapeError` per round-4 P1 #3 — raised by `load_demos_for_skill` for demos that violate the portable `{input, output}` shape or cannot be remapped to the target agent's signature)
...
- `InProcessRunner`: load the script file via `importlib.util.spec_from_file_location` + `spec.loader.exec_module`, then call `module.main(args)`. **Contract parse rule:** `shlex.split(script.run)` must yield `["python", "<relative-path.py>", ...]` (or `["python3", ...]`); the first token is ignored (interpreter is the current process), the second token is resolved to `skill.directory / <relative-path.py>` (must stay under `skill.directory.resolve()` after `Path.resolve()` — reject path-traversal attempts with `SkillScriptError`), remaining tokens are ignored (runtime args come from the validated `args: schema`, not the shell string). Any other shape (e.g. `bash scripts/foo.sh`, `node scripts/foo.js`) raises `SkillScriptError("InProcessRunner requires 'run: python <path.py> [...]'; got <shell-string>. Use flock.sandbox: subprocess for non-Python scripts.")`. Module is loaded with a stable synthetic name `flock.skills.<skill.name>.<script_name>` so repeated invocations hit Python's module cache. Call `main(args)` (coroutine or sync fn) and honor `timeout_seconds` via `asyncio.wait_for`. Raise `SkillScriptError` if `main` is missing, is not callable, or does not accept a single positional arg.
...
- InProcessRunner contract: `run: python scripts/foo.py` → resolves to `skill.directory/scripts/foo.py`, loads via `spec_from_file_location`, calls `main(args)`
- InProcessRunner contract: `run: python3 scripts/foo.py --flag` → same resolution; trailing `--flag` is ignored (args come from `schema`)
- InProcessRunner contract: `run: bash scripts/foo.sh` → `SkillScriptError` (non-Python interpreter; points user at `flock.sandbox: subprocess`)
- InProcessRunner contract: `run: python ../../../etc/passwd.py` → `SkillScriptError` (resolved path escapes `skill.directory` — path-traversal guard)
- InProcessRunner contract: script file loads but has no `main` callable → `SkillScriptError("Script X exposes no main(args) callable")`
- InProcessRunner contract: repeated invocations of the same script hit `sys.modules` under the synthetic name `flock.skills.<skill>.<script>` (module is loaded once, reused)
...
- `load_demos_for_skill(skill: Skill, agent: Agent) -> list[dspy.Example]` — reads JSONL in the **canonical portable shape** defined by `.sfd/contracts.md`: each line is `{"input": <payload-or-dict-of-payloads>, "output": <payload>}`. **The literal keys `input` / `output` are the stable authoring contract** — they are intentionally NOT the agent's semantic field names, so a skill's demos stay portable across agents with different `.consumes/.publishes` shapes. At attach time, `load_demos_for_skill` rewrites each demo against the target agent's signature: (a) introspect the agent's semantic input field names (via `SignatureBuilder.prepare_signature_for_output_group` conventions or equivalent — `Task` → `"task"`, pluralized for fan-out/batch, prefixed `input_`/`output_` on collisions); (b) for single-input agents, map `demo["input"]` (may be a flat dict) directly onto the one input field name; for multi-input agents, require `demo["input"]` to be a dict keyed by canonical type name (`{"Document": {...}, "Guidelines": {...}}`) and fan it out to the per-type semantic field names (`document=..., guidelines=...`); (c) map `demo["output"]` onto the agent's output field name — for skills declaring `flock.outputs`, that single output field is the one matching `outputs_model`'s semantic name; skills without `flock.outputs` use the single-output semantic name from `agent.outputs`; reject multi-output agents (which have more than one semantic output field) with `SkillDemoShapeError("skill demos cannot be auto-attached to agents with multiple publish types — declare flock.outputs or simplify the agent's publishes")`; (d) construct `dspy.Example(**remapped).with_inputs(*semantic_input_keys)`. Raise `SkillDemoShapeError` if a demo line is missing `input` / `output` keys, or if multi-input `demo["input"]` is missing a required type key.
- `validate_outputs_compatibility(skill, agent_output_types: set[type[BaseModel]]) -> None` — raises `SkillOutputMismatchError` if skill declares `outputs_model` but `agent_output_types` doesn't include it (or a subclass). Caller derives the type-set from `agent.outputs` (the `list[AgentOutput]` runtime state on `Agent`, populated by the `.publishes(*types)` builder method): `agent_output_types = {out.spec.model for out in agent.outputs}` — `AgentOutput` exposes the class via `spec: ArtifactSpec` (see `src/flock/core/agent.py:44-88`); there is no `AgentOutput.artifact_type` attribute. (Note: `Agent` has no `agent.publishes` set attribute either — `publishes` is a builder method; the runtime state is `agent.outputs`.)
...
- `load_demos_for_skill`: single-input agent `.consumes(Task).publishes(Report)`, demo `{"input": {...task...}, "output": {...report...}}` → `dspy.Example(task={...}, report={...}).with_inputs("task")`
- `load_demos_for_skill`: multi-input agent `.consumes(Document, Guidelines).publishes(Report)`, demo `{"input": {"Document": {...}, "Guidelines": {...}}, "output": {...}}` → `dspy.Example(document=..., guidelines=..., report=...).with_inputs("document", "guidelines")`
- `load_demos_for_skill`: skill declares `flock.outputs: InvoiceExtracted`, agent `.publishes(InvoiceExtracted)` → demo output field maps to the semantic name of `InvoiceExtracted` (e.g. `"invoice_extracted"`)
- `load_demos_for_skill`: portability check — same demo JSONL produces correctly-keyed `dspy.Example`s for two agents with different `.consumes/.publishes` shapes (authored once, reused)
- `load_demos_for_skill`: multi-input demo missing a required type key (e.g. only `{"Document": ...}` for a `.consumes(Document, Guidelines)` agent) → raises `SkillDemoShapeError`
- `load_demos_for_skill`: demo line missing `input` or `output` key → raises `SkillDemoShapeError`
- `load_demos_for_skill`: multi-output agent (`.publishes(A, B)`) with a skill that does not declare `flock.outputs` → raises `SkillDemoShapeError`
...
- `AgentBuilder.with_skills(*sources, runtime=False, token_budget=8000) -> AgentBuilder`:
  ...
  4. Validate each skill's `outputs_model` against `{out.spec.model for out in self._agent.outputs}` via `validate_outputs_compatibility` (note: `AgentOutput.spec: ArtifactSpec` exposes the class via `spec.model`; no `AgentOutput.artifact_type` attribute exists — see `src/flock/core/agent.py:44-88`. `agent.outputs` is the runtime state populated by the `.publishes(*types)` builder method; there is no `agent.publishes` attribute.)
  ...
  9. Stash demos: `self._agent.skill_demos = [ex for skill in compiled_skills for ex in load_demos_for_skill(skill, self._agent)]` — `None` if no skills have demos. `load_demos_for_skill` handles the portable `{input, output}` → semantic-field-name remap against the agent's signature (see Unit 4's approach description for the remap rules); demos authored once are reusable across agents with different `.consumes/.publishes` shapes.
...
- Create: `src/flock/cli_app.py` — `resolve_flock_app(app_ref, env) -> Flock` helper consumed by every `skills` subcommand; handles `--app module:attr` + `FLOCK_APP` env var + validation (see Approach §"App bootstrap")
...
- Test: `tests/test_cli_app_bootstrap.py` — `--app`/`FLOCK_APP` parsing + import + type-check

**Approach:**
- **App bootstrap (CLI entrypoint for Unit 8 commands).** The CLI needs a live `Flock` instance to introspect (`flock.agents`, `flock.store`, `flock.type_registry`). `src/flock/cli.py` today only knows how to build the demo orchestrator (`from flock.examples import create_demo_orchestrator` at line 41) — no user-project bootstrap exists. Unit 8 adds the convention:
  - **`--app module:attr` flag** on every `skills` subcommand (Flask/FastAPI/Uvicorn convention; e.g. `flock skills optimize invoice-extractor --app myproject.main:flock`). The handler calls `importlib.import_module(module)` and `getattr(module, attr)`; validates the result is a `Flock` instance; raises a `typer.BadParameter` on any mismatch with a clear message ("`--app myproject.main:flock` — expected a `Flock` instance at `myproject.main.flock`, got `<Foo>`").
  - **`FLOCK_APP` env var** as a fallback when `--app` is omitted (same `module:attr` shape).
  - **No auto-discovery, no `flock.yaml`, no convention-over-configuration.** If neither `--app` nor `FLOCK_APP` is set, the command exits non-zero with a message pointing at both options. Explicit is the MVP contract; `flock.yaml`-style discovery can be added later if operator feedback demands it (deferred).
  - **Bootstrap helper** `src/flock/cli_app.py::resolve_flock_app(app_ref: str | None, env: dict[str, str] | None = None) -> Flock` encapsulates the parse + import + attr + type-check logic so the same helper serves every `skills` subcommand (and any future CLI command that needs the user's app). Error class: reuse `typer.BadParameter` — no new skills error; this is CLI-surface concern, not skills-domain concern.
  - **Test scenarios** (add to `tests/test_skills_optimize.py` or a new `tests/test_cli_app_bootstrap.py`):
    - `--app myproject.main:flock` where `myproject.main.flock` is a `Flock` instance → returns the instance
    - `--app myproject.main:flock` where the attr exists but is a `str` → raises `typer.BadParameter` with "expected Flock instance"
    - `--app myproject.main:flock` where the module import fails → raises `typer.BadParameter` with the original `ImportError` in the message
    - `--app myproject.main:missing` where the attr is missing → raises `typer.BadParameter` with "no attribute `missing`"
    - `--app` omitted, `FLOCK_APP=myproject.main:flock` set → resolves from env
    - `--app` omitted, `FLOCK_APP` unset → raises `typer.BadParameter` with both options shown
    - Malformed ref `"myproject.main"` (no `:attr`) → raises `typer.BadParameter` with the `module:attr` shape in the message
...
- `cli.optimize_cmd(skill_name, app_ref, since, success_signal, optimizer, apply, save_as_candidate, output, include_trainset, allow_thin_trainset)` — Typer command; resolves the target `Flock` instance via `resolve_flock_app(app_ref, os.environ)` as the first step, then builds the trainset against it. Default behavior: build diff, present to stdout, prompt `[y/n/d(iff)/s(ave-as-candidate)]`. Flags: `--app module:attr` (required unless `FLOCK_APP` is set in the environment); `--include-trainset` opts into writing raw `dspy.Example` data into history (default: history records only score deltas + SKILL.md diff + config); `--allow-thin-trainset` bypasses the no-upstream `drop_threshold` check.
