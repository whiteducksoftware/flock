...
- **Guard framework:** `src/flock/components/agent/guard.py` — `GuardComponent` abstract base class with `scan_input(text, documents)` / `scan_output(text)` interface and `GuardVerdict` result model. Any custom guard subclasses `GuardComponent` and implements `scan_input()`. Built-in: `AzurePromptShieldGuard` (Azure Content Safety API). Guards are optional — external agents work without them, but operators can attach one or more per agent for defense in depth.
...
- **External agent sandboxing (production requirement):** All adapters spawn processes with maximum permissions (`--dangerously-skip-permissions`, `--full-auto`, `--allow-all-tools`). This means a malicious artifact payload becomes an arbitrary-code-execution vector on the host. Mitigations: (1) production deployments MUST run external agents in isolated environments (container, VM, or dedicated user with restricted filesystem), (2) prefer `--allowedTools` whitelist over blanket permission skip where adapter supports it, (3) validate `working_dir` against an allowlist, (4) optionally attach `GuardComponent` instances to external agents for application-level prompt scanning before spawn. `GuardComponent` is an abstract interface (`scan_input(text, documents) -> GuardVerdict`) — operators can use the built-in `AzurePromptShieldGuard`, implement their own custom guard (e.g., regex-based blocklist, local LLM classifier, or third-party content safety API), or skip guards entirely. Guards are opt-in, not required. This is an application-level mitigation complementing the deployment-level isolation.
...
- Agent extension: Add `agent_kind: Literal["internal", "external"]` to agent model. External agents skip engine evaluation. The `AgentBuilder` fluent API needs extension for external agent registration — directional shape: `flock.agent("reviewer").kind("external").adapter("claude_code").consumes(PRDiff).publishes(ReviewResult).session_mode("resume").guard(MyCustomGuard())` — guards are optional; omit `.guard()` for no scanning. Any `GuardComponent` subclass works: built-in `AzurePromptShieldGuard`, a simple regex blocklist, or a custom implementation calling your own safety API.. This surfaces design decisions: does adapter config go on the agent or is it global? Does `.session_mode()` go on the builder or the subscription? Resolve during implementation, but the DX must feel as natural as internal agent registration.
...
- When guards are attached, scanning runs before spawn; blocked spawns produce audit-worthy events. No guards = no scanning overhead.
...
- **Guard component reuse (optional, pluggable):** The `GuardComponent` abstract interface (`scan_input`/`scan_output` → `GuardVerdict`) is reused for external agents, but called explicitly by the scheduler rather than via lifecycle hooks. No new guard infrastructure needed — `GuardVerdict`, `GuardBlockedError`, and `GuardComponentConfig` (block/warn/annotate) work as-is. Custom guards subclass `GuardComponent` and implement `scan_input()`. Built-in `AzurePromptShieldGuard` is one option; operators can write their own (regex blocklist, local classifier, third-party API) or skip guards entirely.
...
| **Malicious artifact payload → arbitrary code execution** | Defense in depth: (1) optional `GuardComponent.scan_input()` pre-spawn detects prompt injection/jailbreak (plug in Azure Prompt Shield, custom regex guard, local classifier, or any `GuardComponent` subclass), (2) production MUST use process isolation (container/VM/dedicated user), (3) prefer `--allowedTools` whitelist over blanket permission skip, (4) validate `working_dir` against allowlist. |
