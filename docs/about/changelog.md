...
- `Flock.shutdown()` now cancels in-flight agent tasks (bounded by `cancel_grace`) before closing MCP connections.
- Scheduled-agent timers start with the first `publish()` as well as with `run_until*()`.

### 🐛 Fixes

- Orchestrator components are initialized exactly once (previously twice after `add_component()` or timer auto-registration).
